...
_ALIASES = {
    ...
    "name": ("name", "品名", "货物名称", "材料名称", "名称", "description", "item description", "commodity"),
    "spec": ("spec", "规格", "型号", "specification", "model"),
    "package_count": ("箱数", "包装数", "包装数量", "number of packages", "no of packages", "packages", "cases", "cartons", "package count"),
...
def _number(raw, field, unit):
    ...
    if field in COUNT_FIELDS:
        if value != int(value) or suffix not in ("", "pcs", "pc", "ea", "件", "个", "箱", "packages", "cartons", "cases"):
...
def _pdf(data, ocr_backend):
    ...
    with pdfplumber.open(BytesIO(data)) as pdf:
        ...
        for number, page in enumerate(pdf.pages, 1):
            ...
            for table in found:
                rows = []
                for ri, (values, geometry) in enumerate(zip(table.extract(), table.rows), 1):
                    rows.append([_cell(value, page=number, row=ri, column=ci + 1, bbox=list(geometry.cells[ci] or table.bbox), coordinate_system="pdf_points") for ci, value in enumerate(values)])
                tables.append({"rows": rows, "source": {"page": number, "bbox": list(table.bbox), "coordinate_system": "pdf_points"}})
    if scan_pages:
        if ocr_backend == "none":
...
def _table_rows(tables, issues):
    ...
    for table in tables:
        ...
        for cells in matrix:
            ...
            if len(set(fields)) >= 2 and any(f in fields for f in ("name", "package_id", "material_id")):
            ...
            summary = first_written if _norm(first_written) in ("合计", "总计", "小计", "本页合计", "total", "subtotal", "grandtotal") else None
            if summary:
                grand = _norm(summary) in ("总计", "grandtotal")
                totals.append({"label": summary, "values": {f: row[f] for f in NUMERIC_FIELDS if type(row[f]) in (int, float)}, "source": next((c.get("source", {}) for c in cells if str(c.get("raw", "")).strip() == summary), table.get("source", {})), "row_ids": list(all_ids if grand else section_ids)})
                section_ids = []
                continue
