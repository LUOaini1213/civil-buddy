...
def try_place(bin: Bin3D, item: Item3D) -> Optional[Placement3D]:
    ...
    # 候选点：extreme points + 地面双列网格 + 顶面堆叠（激进）
    pts = list(bin.points)
    for dx0, dy0, _ in _orientations(item):
        cy = max(0, (bin.W - dy0) // 2)
        pts.append((0, cy, 0))
        step = max(min(dx0, 500), 200)
        x = 0
        while x + dx0 <= bin.L:
            pts.append((x, 0, 0))
            pts.append((x, cy, 0))
        ...
        for px, py, pz in pts:
            ...
            if pz > 0:
                if item.prefer_bottom or not item.stackable:
                    continue
                # 二层仅堆一层（z 对应底层箱顶，不再三层）
                if pz + dz > bin.H:
                    continue
            center_y = py + dy / 2.0
            center_x = px + dx / 2.0
            new_moment = abs(moment + w * (center_y - mid_y))
            long_pen = float(px) * 0.02 + abs(center_x - mid_x) * 0.05 * min(w / 800.0, 1.5)
            if first:
                y_score = float(py)
                bal = 0.0
            else:
                edge = min(py, max(bin.W - (py + dy), 0))
                y_score = float(edge)
                bal = new_moment
            residual_h = bin.H - (pz + dz)
            height_waste = residual_h * 0.001
            # 可堆叠矮箱：略鼓励二层（pz 权重更低）；底层件强制 pz=0 已在上面 continue
            z_weight = 0.15 if (item.stackable and not item.prefer_bottom) else 0.5
            cand = (
                pz * z_weight,
                bal,
                y_score,
    ...
    if best is None:
        return None
    _, _, _, _, _, _, dx, dy, dz, x, y, z = best
