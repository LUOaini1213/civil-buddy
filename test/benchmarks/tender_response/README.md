...
- 解析器抽不出来的招标句没法被对照。可以写进 case，评测会把落在这种句子上的 link 计入 `gold_on_lines_with_no_row`，召回率照样按全部 link 算，不美化。「项目经理须具备一级注册建造师资格」「投标有效期90天」不带 ★ 时曾经就是这样：2026-09-20 把它们和最高限价、质量标准、质保期、付款条件、BCA workhead 一起写成 5 个 case，当时 `gold_on_lines_with_no_row=11`、link R 0.765；补了 `tender_parse._RULES` 之后是 0 和 0.980。
