...
### 钢结构 Excel 测试集（REDACTED-CLIENT多 sheet 拆分 + 合成）

网上几乎没有「材料→铁架→拼柜」同业务完整 Excel。以项目内REDACTED-CLIENT表为主：

```bash
python scripts/build_steel_test_set.py   # 从 REDACTED-CODE*REDACTED-CLIENT*.xlsx 拆分 → test/excel/
...
| **REDACTED-CLIENT Excel** | **业务正确性 / 答辩主案例** |
