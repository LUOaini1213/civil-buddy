...
VMU1 = Path(r"A:\JOB\2517SLTO\Project\6. Quality QAQC\6.06 POR\VMU\VMU1")
...
def load_bgl0003() -> List[Dict[str, Any]]:
    x = Path(
        r"A:\JOB\2517SLTO\InterDepartment\3.05 Action\User Working files"
        r"\Simon Ng\VMU\MATERIAL ORDER\Glass"
        r"\SLTO-VMU-0001-BGL0003 (VMU01 玻璃提料单-送工厂）.xlsx"
    )
    mats = []
    if not x.exists():
        return mats
...
def load_fac0007() -> List[Dict[str, Any]]:
    x = Path(
        ...
        r"\007-SLTO-VMU01-FAC0007(VMU01F HONEYCOMB MATERIAL TAKE OFF-SEND TO FACTORY.xlsx"
    )
    mats = []
    if not x.exists():
        return mats
