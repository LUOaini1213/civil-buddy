...
def _find_source(explicit: str = "") -> Path:
    ...
    cands = list(ROOT.glob("SLT0*.xlsx")) + list(ROOT.glob("*远东*.xlsx"))
    if not cands:
        raise FileNotFoundError("未找到远东 Excel（SLT0*.xlsx）")
    return cands[0]
...
def parse_packing_sheet(ws, sheet_name: str) -> List[Dict[str, Any]]:
    ...
    for row in rows:
        ...
        # 标题行：含项目名的完整铁架标题
        if m and ("远东" in text or "SLTO" in text or "号" in text):
...
def main() -> int:
    ...
    ap.add_argument("--source", default="", help="远东 Excel 路径")
    ...
    write_multi_sheet(
        out_dir / "test_full_flow.xlsx",
        {
            ...
            "meta": (
                ["key", "value"],
                [
                    {"key": "project", "value": "远东新加坡陆路交通局办公楼"},
    ...
    schema = {
        ...
        "synthetic_cases": {
            ...
            "syn_mixed_realistic": "长短混装，贴近远东",
        },
        "source_excel": src.name,
        "note": "网上同业务完整 Excel 几乎没有；以本项目远东表 + 合成为主",
    }
