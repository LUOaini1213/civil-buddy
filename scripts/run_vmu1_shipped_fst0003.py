#!/usr/bin/env python3
"""
VMU1 已发货：解析 FST0003 装货单 1/2 柜 → 订柜 N0 + 3D can_fit。
只读 A: 已发货目录，不改源文件。
"""
...
SHIPPED = Path(r"A:\JOB\2517SLTO\Project\6. Quality QAQC\6.06 POR\已发货")
...
def main() -> int:
    ...
    files = list(SHIPPED.rglob("*FST0003*"))
    if not files:
        print("NO FST0003 shipped file under", SHIPPED)
        return 1
    path = files[0]
    print("FILE", path)
    ...
    md.append(f"\n产物：`{outj}`")
    mdp = OUT / "VMU1_已发货_FST0003_装柜复算.md"
