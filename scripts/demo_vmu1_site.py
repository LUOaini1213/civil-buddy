...
用法（仓库根目录）:
  ...
  python scripts/demo_vmu1_site.py --with-shipped   # 附带已发 REDACTED-REF 复算
...
def assemble_package(*, with_shipped: bool) -> Path:
    ...
    if with_shipped:
        for name in (
            "VMU1_已发货_REDACTED-REF_装柜复算.md",
            "vmu1_shipped_REDACTED-REF_pack.json",
        ):
            src = OUT_SHIP / name
            if src.exists():
                shutil.copy2(src, pack / name)
...
| 已发 REDACTED-REF | 2 柜（装货单） | 对照样例，非写死约束 |
...
- `images/` — 最近侧视图（若本机曾出图）
- 可选：`VMU1_已发货_REDACTED-REF_装柜复算.md` — 已发 2 柜复算
...
python scripts/run_vmu1_shipped_REDACTED-REF.py   # 可选已发
```
...
def main() -> int:
    ...
    ap.add_argument("--with-shipped", action="store_true", help="同时复算已发 REDACTED-REF")
    ...
    if not args.skip_run:
        ...
        if args.with_shipped:
            rc2 = _run([sys.executable, "scripts/run_vmu1_shipped_REDACTED-REF.py"], timeout=300)
            if rc2 != 0:
                print("WARN shipped script failed", rc2)

    pack = assemble_package(with_shipped=args.with_shipped or (OUT_SHIP / "VMU1_已发货_REDACTED-REF_装柜复算.md").exists())
