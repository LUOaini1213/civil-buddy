---
name: hydraulic
description: "水利（勘察设计）：堤防、护岸、水闸、泵站提纲，无水文地质不定量。交付水利设计提纲。适用于水利工程、堤防、水闸。内部草稿。"
metadata:
  category: "design"
  category_name: "勘察设计"
  title: "堤防、护岸、水闸、泵站提纲，无水文地质不定量"
  delivers: "水利设计提纲"
  risk: "high"
  aliases: "水利工程,堤防,水闸"
---

# 水利

Civil Buddy 的勘察设计专业技能；程序记忆（Skill / SOP），不是用户画像或规范全文。
堤防、护岸、水闸、泵站提纲，无水文地质不定量。默认交付：水利设计提纲。风险：high。

可以只聊天。只有用户本轮要求成稿才调用写入工具；缺少资料可列空栏骨架，不能把待核输入变成事实。

## 成稿资料

按本次任务核对以下资料；缺项逐项标 [A001] / UNSPECIFIED，招标未载内容标「招标未写」。

- 辖区（未提供则UNSPECIFIED；多个对象单独归属）
- 水工建筑物或区段编号
- 水文水位和流量资料
- 地质及基础条件
- 堤防护岸水闸泵站范围
- 设计任务及上下游接口

## 专属步骤

1. 按建筑物登记水文地质与边界条件。
2. 编排防洪排涝、结构防渗和机电原则。
3. 列水力稳定计算与运行管理提资缺项；调用 `hydraulic__outline`。

## 输出结构

交付完整 Markdown 草稿及可打开的 Word；正文有表格时可导出 Excel。文首保留内部草稿声明，签认栏由持证人员填写。

1. 范围声明
2. 水文
3. 地质
4. 堤防护岸
5. 水闸泵站
6. 防渗消能
7. 施工导流
8. 运行监测
9. 专业接口

专业表格至少表达以下字段（缺项保留空栏，不能串用其他对象的值）：

- 水工参数：建筑物｜水位｜流量｜地质资料｜来源｜待计算项

## 验收

- 水位流量逐建筑物引用，不按经验补设计频率
- 水力、抗滑及防渗计算结果未连接保持UNSPECIFIED
- 实际生成文件须能打开；用户已给字段进入对应正文或表格，不能仅在附录重复用户原文。

## 能力边界

- 不凭缺失水文选泵或定断面，不替代水利设计文件
- 本岗专属工具不能借给兄弟岗；文书起草工具不等于工程求解器。

## 按需读取

- 需要完整专业细节时读 `demo/kb/design/hydraulic/outline.md`；其中历史规范标题不是已经核实的现行依据。
- 检索仅限本岗 `demo/kb/design/hydraulic/` 与大类 `demo/kb/design/_shared/`。
- 公司规则：`demo/kb/company/hard-rules.md`；法规引用打开对应官方原文再核对版本和条款。

## 共同边界

- 不编条款号、材料强度、岩土参数、综合单价、xyz、柜数、N0。
- 引用写全名+年份+条款；没抽到原文标 unverified / UNSPECIFIED。
- 无来源数字写 [A001] 起待填。
- 禁止断言：可交差、可报审、报审通过、可提交专家论证、请监理审核后开工、可以开工、可以投标。
- 产出是内部讨论 AI 草稿，不是法定签认件。
- 辖区 CN / SG / EU / DUAL 禁止静默混用。以本次明确地区或会话已确认地区为准；未提供则 UNSPECIFIED，不默认国家。DUAL 的依据和接口按明确地区分栏。
- 高风险成稿写盘前，用户须打出：我明白，将由持证人员签认。纯提问不受确认门阻挡。
