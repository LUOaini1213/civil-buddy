...
export function startPlanningApp(document, options = {}) {
  ...
  async function exportBundle() {
    if(state.busy)return;
    try{settleEdits();if(!state.project||dirty())throw new Error('请先保存当前修改，再导出项目包。');if($('confirmation').value!=='我明白，将由持证人员签认')throw new Error('请完整输入签认提示后导出。');}catch(error){notify(error.message,true);return;}
