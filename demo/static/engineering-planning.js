...
export function startPlanningApp(document, options = {}) {
  ...
  const state = {plan:blankPlan(),name:'',weekly:[],project:null,baseline:null,result:null,resultPlan:'',resultStale:false,runId:null,method:'cpm',saved:'',history:[],pending:new Map(),busy:null,sequence:0,view:'gantt',optimization:null,importDraft:null,sourceId:null,importSource:null,synthetic:false};
  ...
  function controls() {
    ...
    $('cancelRequest').disabled=false;$('cancelRequest').hidden=!['calculate','optimize','load','import'].includes(state.busy);
  ...
  async function action(kind, fn) {
    if(state.busy)return;
    state.busy=kind;controller=new AbortController();operationId=['calculate','optimize'].includes(kind)?globalThis.crypto.randomUUID().replaceAll('-',''):null;
  ...
  function renderTasks() {
    ...
    for(const task of state.plan.tasks){
      ...
      for(const [field,label]of [['actual_start','实际开始'],['actual_finish','实际完成']])cell(row,input(task[field],`task:${task.id}:${field}`,`${task.id} ${label}`,(v)=>{const date=optionalDate(v,label);return()=>{task[field]=date;};},{type:'date',min:'1900-01-01',max:'2200-12-31'}));
      cell(row,button('删除',()=>{
  ...
  function renderResult() {
    ...
    const engine=state.result.engine;$('engineStatus').textContent=`${engine?.name||''} ${engine?.version||''}${state.method==='resource'?` · ${state.result.solver_status||''} · ${state.result.proven_optimal?'已证明最优':'可行方案，未声明最优'}。资源方案不报告 CPM 时差或关键路径。`:' · 实际日期仅作记录，不自动锁定或重排剩余任务。'}`;
  ...
  function render() {renderCalendar();renderTasks();renderResources();renderResult();renderBaseline();renderWeekly();renderOptimization();renderSource();controls();}
  ...
  function newLocal(plan=blankPlan(),name='',synthetic=false) {state.plan=clone(plan);state.name=name;state.weekly=[];state.project=null;state.baseline=null;state.result=null;state.resultPlan='';state.resultStale=false;state.runId=null;state.method='cpm';state.history=[];state.pending.clear();state.optimization=null;state.importDraft=null;state.sourceId=null;state.importSource=null;state.synthetic=synthetic;state.saved=plan.tasks.length?'':JSON.stringify(snapshot());$('importPreview').hidden=true;$('confirmation').value='';renderRevisions();render();if(win?.history)win.history.replaceState(null,'',win.location.pathname);}
  ...
  async function exportFile() {if(state.busy)return;try{settleEdits();assertReady();if(!current()||!state.runId)throw new Error(state.method==='resource'?'资源方案已修改或缺少有效计算记录。请先预览资源调整并应用调整，再导出；当前编辑与上次结果已保留。':'请先计算关键路径，取得与当前输入一致的结果后再导出。');if($('confirmation').value!=='我明白，将由持证人员签认')throw new Error('请完整输入签认提示后导出。');}catch(error){notify(error.message,true);return;}return action('export',async(token)=>{const format=$('exportFormat').value,payload={plan:state.plan,format,confirmation:$('confirmation').value,run_id:state.runId};const response=await request('/export',payload,token),blob=await response.blob();if(!owns(token))return;const url=URL.createObjectURL(blob),link=el('a');link.href=url;link.download=`${state.name.replace(/[<>:"/\\|?*\x00-\x1f]/g,'_')||'planning'}.${format}`;document.body.append(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);notify('导出文件已生成，包含当前明确输入。请重新打开并核对交接结果。');});}
  ...
  return{state,ready,calculate,optimize,save,open,undo,cancel,example,importFile,applyImport,exportFile,restoreRevision,captureBaseline,newLocal,render,current,dirty,dispose(){controller?.abort();state.sequence++;win?.removeEventListener?.('beforeunload',unload);}};
}
