...
| Tender vs. bid response, measured | One row per tender line; candidate response lines are matched and a stated number that does not meet the tender's (工期 60 vs 999 日历天) is flagged for review, never judged. Scored against 14 hand-labelled cases: link precision 1.000, recall 0.974, 9/9 numeric mismatches, no duplicate rows (the previous matcher: 0.857 / 0.615 / 0 of 9) | `python scripts/eval_tender_response_match.py --variant all` |
