...
| ⑫ 2026-09-17 表格读取静默错误的清剿，先量化再改（`28d70a1`、`60fdb87`、`3735aae`、`2bedcc9`） | 人定"先量化再改"：每处修复必须先有基准用例集和改前的分数，不许凭感觉改完就说好了 | AI 先建基准（`test/benchmarks/tender_response/cases.json`，14 例 39 条链接）再逐项修：`L (mm)` 类写法 8 种识别不了 5 种，行进来是 0×0×0 还报成功；英寸/英尺/磅与合并尺寸格 16 种写法错 12 种；招标响应比对把同一句重复三遍且从不比数字 | 当时 14 例的改前改后：精确率 0.857→1.000、召回 0.615→0.974、9 处数字冲突从 0 检出到 9 全检出、每行候选 1.316→1.000。那是 2026-09-17 那一版，不是现在的 `--check`。被否掉的方案也留数字（bigram≥2 精确率只有 0.907）——否决理由同样要可复核 |
...
| ⑫ 招标响应比对 | `python scripts/eval_tender_response_match.py --check`（现行 19 例：链接精确率 1.000 · 链接召回 0.980 · 冲突精确率 1.000 · 冲突召回 1.000，14/14。2026-09-17 的 14 例召回 0.974、9/9 是当时的改前改后，不是这次的分数） |
