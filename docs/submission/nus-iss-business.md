...
- the tender check lists the CR16 workhead, the 420-day duration, the validity period, the performance bond, the defects-liability period, all 3 rejection clauses and the 4 evaluation weightings, each with its clause or section reference, and leaves 10 rows it could not find for a person to check;
- a list of 24 unitised panels becomes 6 × 40HQ containers that fit, with 24 → 24 pieces and 10,800 → 10,800 kg conserved;
- the high-risk work-at-height briefing writes nothing until a person types the sign-off sentence [5].

Two gaps from our first façade tests were addressed on 26 September 2026 (pull request #63). English requests are now routed to a post instead of always ending as chat: on a blind held-out set, routing scores 0.786 accuracy with 0 false runs [6]. And the English proposal's compliance schedule no longer claims "No Deviation" for clauses a packing run cannot answer: it leaves those packaging and transport clauses to a person and marks a lashing clause at most "Partial Deviation" [7].

The demo also prints what is still missing: the tender check does not yet list the eight façade specification clauses (mock-ups, heat soak, site water test, PE-endorsed submissions, warranty, A-frame delivery, insurance), liquidated damages or retention, and the packing plan ignores glass-handling notes. Façade tender topics and glass-handling notes are being added before the demo on 10 October 2026. Stillages for glass and flat-rack containers are not planned yet (§4).
...
**What is measured, and on what.** The façade figures below come from our façade demo, which anyone can rerun from the repository: `python scripts/demo_facade.py` runs civil-buddy at `916971d` offline, in its default no-key mode, on the synthetic files in `examples/facade-demo/`. They are a Singapore façade subcontract enquiry, a list of 24 unitised panels with English and with Chinese handling notes, and installation daily-report and work-at-height inputs [5]. The other figures are from the Technical Document [6] and the repository [7]. None is partner data. **We have not measured staff time saved.** Every time figure is machine time; time saved is a pilot metric. Each "before" line is the team's understanding of current practice, not something observed at the partner.
...
- On the synthetic façade enquiry (four sections and an appendix table), the tender check lists, each with its clause, line or section reference: the tender reference, the closing date, the CR16 workhead, 420 calendar days, 90-day validity, a 10% performance bond by banker's guarantee, a 12-month defects-liability period, "alternative tenders not permitted", all 3 rejection clauses, and the 4 weightings of Section 3. It also notes that the text names the Price-Quality Method, without a clause. The 10 rows it could not find are marked "not found in the text" for a person to check against the enquiry [5].
- From the same hand-off, the technical-bid post drafts 4 chapters from the 4 scoring points, with 13 cells left for the bid team, and the compliance post lists 8 requirements with no response yet and the 3 rejection clauses to tick by hand [5].
...
- It can also draft an English Singapore façade "Contractor's Proposal" in which every price, UEN and track-record cell stays `[TO FILL]` for a person [7].
- **The proposal's compliance schedule no longer marks what a packing run cannot answer "No Deviation"** (fixed on 26 September 2026). Short codes now match whole words only, so the letters "CTU" inside "structural" no longer turn a mock-up-test or PE-endorsement clause into cargo securing. A packing run counts as evidence for a container clause only if it used the container type the clause names, and never for lashing (a lashing clause reads at most "Partial Deviation"). Clauses on handling the packing engine does not model (A-frame, stillage, upright, no stacking, fragile) are left to a person ("Pending SME") instead of "No Deviation". A release-gate check (`facade-tender`) holds these rules [7]. Not fixed yet: the tender-delivery run (tender check plus packing plan) can choose 20GP containers when 40HQ was requested; its compliance matrix then leaves that container clause to a person [7].
...
**Shown by the demo as not done yet, and being added before 10 October.**

- None of the eight façade-specification clauses (laboratory mock-up, visual mock-up, heat soak, site hose test, PE-endorsed calculations and shop drawings, warranties, A-frame delivery, insurance) reaches the review table: the demo counts 0 of 8, and nothing in the draft says they were left out. Liquidated damages and retention in the appendix table are not shown either, and the qualification row is among the 10 left "not found", although the enquiry asks for bizSAFE Level 3 [5]. **Being added:** façade tender topics in English and Chinese, measured on a held-out façade enquiry written before the first run (the demo's enquiry has been seen, so a score on it is a development figure).
- The review table is in Chinese, although English commands are now routed to a post (§4.3).
...
- On a synthetic list of 24 unitised panels (4,200 × 1,500 × 250 mm, 450 kg each, 10,800 kg in all), the engine puts each panel in its own crate (24 crates) and plans 6 × 40HQ containers that fit; the booking lower bound is also 6. Conservation holds: 24 → 24 pieces and 10,800 → 10,800 kg. The structure verdict for all 24 crates is "pending detailed design": without structural facts, the engine does not invent a pass. Lashing stays unspecified [5].
...
- The plan is conservative: one panel per crate, and the six containers use 44% of their space and carry 9% of their rated payload [5].
- Handling notes change nothing: the plan is the same with the Chinese notes "glass, fragile, do not tip, transport upright, do not stack", with English notes, and with the notes removed. The enquiry asks for upright delivery on A-frame stillages, which the engine does not model [5].
...
**Not planned yet:** A-frame or stillage crates, which need the partner's real stillage dimensions, tare weight and capacity (anything else would be invented numbers) [5]; 40-foot flat-rack and open-top containers, which the engine does not support (it plans only 20GP, 40GP, 40HQ and 45HQ) [7]; and more than one container type in a plan [5]. **Pilot measures:** containers planned against containers actually shipped, and pieces and kilograms conserved from list to plan.
...
- The high-risk façade post, and the safety-brief, method-statement and hazard posts, write nothing until a person types the sign-off sentence. In the demo, the work-at-height briefing replies that nothing was written and names the sentence a licensed person must type; the script never types it [5]. This holds in the desktop app, the terminal UI and the workbench page, and, since 26 September 2026, over the web API and the MCP server too.
- From Chinese "label: value" lines for one façade installation day, the daily-report draft puts each labelled fact in its own row: project, date, weather, location, progress (6 of 24 panels), attendance (13), plant and panels delivered, the gasket defect and the checked work-at-height permit, and tomorrow's plan. The preparer and reviewer are left blank for a person [5].
- **English requests** (since 26 September 2026). In the default mode, many English requests are now routed to a post instead of always ending as chat. On a blind held-out set of 28 English sentences, written after the rules were frozen, routing scores 0.786 accuracy with 0 false runs: no question or remark was turned into a written file. The 6 misses, mostly daily-report, site-diary and toolbox-talk requests, fell back to chat [6].
...
**Still missing, as the demo shows.**

- An English sentence still places almost nothing in the draft, even when it reaches the right post; the demo's daily-report and briefing inputs are Chinese labelled lines for that reason [5].
- Once signed, the work-at-height briefing is still a generic body with its cover fields left to fill; only its sign-off gate is façade-ready [5].
...
**Being added before 10 October:** an English façade plugin with three forms: a Façade Installation Daily Report, a Work-at-Height Toolbox Briefing and a Panel Installation Method Statement outline. Its knowledge will hold only sourced facts; the first of them, such as BCA's CR16 workhead, MOM's 3 m permit threshold and BCA's Periodic Façade Inspection, are already in the knowledge base of the façade, safety-brief and hazard posts, each with its source and date [7]. As a plugin it asks for the sign-off sentence on every write until the firm trusts it. **Pilot measures:** fields placed on the partner's own daily-report and permit forms.
...
**Why this segment first.** [TEAM TO FILL: confirm.] CR16 is the workhead for supplying and installing curtain walls [14], so its firms tender for façade work and install at height, and those that use unitised panels also crate and ship them: the partner's three jobs. The segment is a known public list, and the partner's pilot gives the evidence a second firm will ask for. English comes first, since these firms tender in English: English commands are now routed to a post (§4.3), while the interface, the sign-off sentence and fact placement from English input are still to come. We give no dollar market size until a price is chosen (§8); after that, the serviceable market is firms × annual price. The PFI regime, with about 30,000 buildings expected in its first cycle [22], brings inspection and repair work on existing façades; civil-buddy has no post for it, and we list it only as a later possibility.
...
| **Façade gaps shown by our demo** | Stated in §4: façade specification clauses, liquidated damages and retention not yet listed; handling notes ignored; no stillages. The compliance schedule no longer marks clauses a packing run cannot answer "No Deviation" (fixed 26 Sep 2026) | Fixes before 10 Oct (§4, §13); stillages only with the partner's real data |
...
| **Not yet English-ready** | Chinese interface and sign-off sentence; English commands are routed (0.786 on a blind held-out set, 0 false runs), but an English sentence places almost nothing in a draft; English tenders measured only on synthetic sets | [TEAM TO FILL: English interface and sign-off sentence by when?] |
...
The platform rows match the Technical Document §7, which also puts the audit log and metering after the demo. The façade rows come from our façade demo [5]. The Technical Document describes the demo (§3.4) but does not schedule the façade rows.
...
5. Team Mintang. Façade demo in the civil-buddy repository at `916971d` (pull request #63, merged 26 September 2026): `examples/facade-demo/` (synthetic fixtures: a Singapore façade subcontract ITT, 24 unitised panels as English and Chinese spreadsheets, daily-report and work-at-height inputs; its README lists what the demo does not show yet) and `scripts/demo_facade.py`, which runs the three flows through `civil exec`'s entry point offline, in the default no-key (steps) mode, and prints what each flow extracted, what is missing and where a person must sign. Figures from our run on 26 September 2026; reproducible with `python scripts/demo_facade.py`. Every file is synthetic; none comes from the partner. <https://github.com/LUOaini1213/civil-buddy>
6. Team Mintang. *civil-buddy: Technical Document* (NUS-ISS submission), code baseline `main` at `d3ada11`, with the façade demo and English routing of pull request #63 (§3.4) measured at `916971d`, 26 September 2026. <https://github.com/LUOaini1213/civil-buddy>
7. Team Mintang. civil-buddy repository at `916971d` (26 September 2026), MIT licence: security baseline, pull request #61, merged 26 September 2026 (GitHub Actions: 142/142 checks passed in run 36170078042 on the pull request and again in run 36171221472 on `main`), with operator settings in `docs/deploy-minimal.md`; pull request #63, merged 26 September 2026 (145/145 checks passed in run 36222056643 on the pull request and again in run 36222613544 on `main`): the compliance-schedule rules pinned by `scripts/test_facade_tender.py` and `scripts/test_tender_delivery_api.py`, the English bid-book in `knowledge_base/08_tender_delivery/strategies/sg_facade_bid_book.md`, the container types the engine plans (`container_inner_mm` in `packing_assistant/knowledge.py`), and the sourced façade knowledge in `demo/kb/design/facade/`, `demo/kb/hse/safety-brief/` and `demo/kb/construction/method-hazard/`; `docs/civil-buddy/real-tender.md`, `plugins.md`, `civil-codex-eval-2026-09-19.md`; `test/benchmarks/verdicts/cases.json` (the recorded `qwen2.5:3b` sentence); `创意材料-工友侧.md` (worker-side material, 31 Aug 2026); `.agents/skills/{worker-brief,safety-brief,hr-labor}/SKILL.md` and `demo/kb/`; tonnes fix `53a550e` (13 Sep 2026). <https://github.com/LUOaini1213/civil-buddy>
