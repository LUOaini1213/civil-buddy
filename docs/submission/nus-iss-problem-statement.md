...
{{SME_NAME}} is a Singapore curtain-wall contractor [TEAM TO VERIFY: SME-track eligibility — use "SME" for the partner only if the organisers confirm], registered with BCA under {{SME_CRS}}. In three of its jobs, a wrong number or verdict is costly. In its own words: [TEAM TO FILL: the partner's own words, the same text as SME_FEEDBACK in sme.local.json, shortened to one sentence].
...
- **The feedback is verbal.** Until the partner confirms in writing, fill the "own words" placeholder only with what it said and agreed to be quoted; otherwise delete that sentence and say in the email that the feedback was verbal.
