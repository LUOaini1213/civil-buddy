...
The organisers' briefing gives seven judging criteria, without weights, and the event page lists six qualities a solution should have. The table maps each one to what civil-buddy does, with the strongest evidence a reader can check: a file, a gate check named as in `npm run check`, a figure from this document, or a pull request. It also gives the section that shows it and the gap we know of. Every figure here is one this document already reports; this section adds no new measurement.

| Criterion | What civil-buddy does, and the evidence | Shown in | Honest gap |
|---|---|---|---|
| **Judging rubric** | | | |
| **1. Goal & Scope Definition** (business value, clear purpose) | One purpose: internal drafts whose numbers and verdicts a small construction firm can trust. Code computes every number, a missing value stays `UNSPECIFIED` or 待填, and every draft carries `submit_blocked = true`. The pilot partner's three façade jobs run end to end on synthetic files with one command (`scripts/demo_facade.py`, pull request #63). | §1, §3.4 | Breadth is ahead of depth: across 65 posts only 0.229 of stated facts land in the right field. No pilot has started, so the business value is not yet measured. |
| **2. Architecture & Reasoning Loop** (planning pattern, explicit state and memory) | Workflow before loop: rules route each request, tender review is a fixed DAG (parse, then technical ∥ compliance, then aggregate), and the opt-in model loop runs only when no workflow matches, for at most 10 steps. State is explicit and on disk: a hashed `handoff.json`, a `check.json` with each input's SHA-256, a LangGraph `SqliteSaver` checkpoint for the packing pause, a revisioned `/logistics` ledger and task memory. An approval never outlives its turn (`memory.py:85`). | §2.2, §3.1, §3.2 | Model-mode behaviour is outside the gate, and no live-model result is claimed (§5.1, §5.4). Sessions are not yet per user (`ROADMAP`). |
| **3. Tool Use & Integration** (purpose-fit tools, well-typed schemas) | 81 registered tools (71 of them write), each with a JSON Schema input and output contract (`runtime/tool_contracts.py`, check `tool-contracts`). An unknown or mistyped argument is refused before policy runs (`tool_engine.py:188-190`); a writer's successful result without `submit_blocked = true` is refused as a contract error (`tool_engine.py:346-347`); MCP advertises the same schemas, minus the approval fields (`demo/mcp_surface.py:124-136`). Integrations: MCP over stdio for VS Code and Cursor, the knowledge base as MCP resources, REST under `/api/mcp/*`, the packing gateway API, Word and Excel export. | §2.2 (tool gateway; MCP server), §4.3 | Not every path passes the engine: the gateway's own packing routes and some model-loop tools call their functions directly, and the undeployed Rust `civil-mcp` has its own tool path (§2.3, rule 1). No per-tool version (rule 3). |
| **4. Autonomy & Human-in-the-Loop** (risk-calibrated autonomy, escalation checkpoints) | Four levels, each set in code: a question writes nothing; a low-risk post drafts on its own, for internal use only; input a machine cannot trust stops for a person (needs-human rows, unreadable files, number conflicts, "Pending SME"); and the 19 high-risk posts write nothing until a person types the sign-off sentence, which no setting removes and which covers one turn. Details below. | §3.1–§3.3, §4.1, §4.3 | No approver is named until accounts land (`BEFORE DEMO`). On the CLI, `--confirm` is the operator's own assertion. A gateway API caller that omits `enable_auto_confirm` skips the packing pause (§4.4). |
| **5. Safety, Security & Guardrails** (prompt-injection resistance, least-privilege access) | Containment by design, not detection: with no key, no model runs; with a key, model text never reaches a deliverable, the model cannot approve, and its numbers and verdicts pass two deterministic guards. Least privilege: policy as code before each registered tool (9 deny codes), per-post exclusive tools, per-launch MCP scopes, every tool's `file_path` checked against the sandbox roots and writes confined to them, and one access guard in front of both web apps. The security baseline (pull request #61) is pinned by the checks `access-guard`, `human-approval`, `http-confirmation` and `pack-ship-read-sandbox`. Details below. | §2.2, §4.1–§4.4 | No prompt-injection detector and no injection test set. One shared token and no user identity. The OS sandbox is opt-in and inherits the host's keys. The undeployed Rust `civil-mcp` and Rust workbench still accept `confirm_ok` (§4.4). |
| **6. Observability & Evaluation** (logging and tracing of decisions, golden-path and adversarial cases) | Every policy refusal carries a deny code and a reason; each run writes a `workbench.json`; a tender check writes `check.json`; a packing run keeps a per-run trace (`packing_assistant/trace_events.py`, check `trace-artifacts`). A 142-check offline gate runs in CI on every pull request and push to `main` (142 of 142 at `d3ada11`, 145 of 145 at `916971d`). Every benchmark set is labelled held-out, dev or seen, and adversarial cases sit beside the golden path. Details below. | §2.2, §5 | The engine's audit log is in memory only, and tool events are not persisted. Run records carry no model name and no cost. OTel is opt-in. The gate covers the deterministic path only, and there is no LLM judge (`ROADMAP`). |
| **7. Platform & Tooling Usage** (idiomatic framework use, clean multi-agent orchestration) | Standard parts in their usual roles: FastAPI behind one access guard; MCP over stdio (JSON-RPC); LangGraph `SqliteSaver` to pause packing Team A for a person's confirmation and resume Team B after a restart; two checkers running in parallel on one frozen handoff, with no shared live state; OpenAI-compatible Chat Completions; SQLite FTS5; Docker; GitHub Actions. On AWS, as designed: one Lightsail instance per company, and Bedrock through the same Chat Completions setting. | §2.2, §3.1, §3.2, §6 | The Lightsail instance is not yet running. Bedrock is configurable but has never been run from this repository, and whether it works from Singapore is not yet known (§6.3). Four model clients, not one egress (`ROADMAP`). |
| **Solutions should be** | | | |
| **Practical & relevant to SME needs** | Built for a firm with a handful of people: no key and no model call by default, on a laptop or one server, through a browser, the CLI, a desktop app or an IDE. On synthetic versions of the partner's three jobs it lists the tender's key terms and rejection clauses, plans 24 panels into 6 × 40HQ with pieces and mass conserved, and drafts the daily report, while the work-at-height briefing waits for the sign-off. | §1, §3.4 | The parse lists 0 of the ITT's 8 façade specification clauses; handling notes do not change the plan, and A-frame stillages are not modelled. The UI and the sign-off sentence are in Chinese (§3.4, §7). |
| **Feasible to deploy or pilot** | What a server needs is on `main`: the gateway's Docker image with a health check, a start that refuses to open without `CIVIL_TOKEN`, SQLite backups (7 kept) and an operator guide for one Lightsail instance (`docs/deploy-minimal.md`). Every post runs offline with no key, so a pilot needs no model account. | §6 | The instance is not yet running; its URL is to follow in the submission email. No TLS proxy configuration file ships, there are no accounts yet, and no pilot has been agreed (§1, §7). |
| **Secure & responsible in design** | Code computes every number, and the 19 high-risk posts need a person's typed sign-off. Verdicts the product may never state ("can bid", "passed review") are struck from model-mode replies by the verdict guard, and `civil review` flags them in any document. With no key, nothing is sent to a model. The limits are listed, not hidden (§4.4, §5.4). | §1, §4 | Nothing is masked before a cloud model call (PII masking `ROADMAP`), so no key or a local Ollama is the safeguard. The open items of §4.4. |
...
| **Effective use of agentic AI** | The model plans and phrases; code computes, checks and writes. Rules route first, a fixed workflow runs two evidence-bound checkers in parallel, and a bounded model loop picks from a fixed tool menu. A model analysis must cite its sources and use only numbers from the cited quotes, or it is rejected (`tender_workflow.py:117-146`). If the model is down, `agent_mode = auto` falls back to deterministic steps. | §2.2, §3.1, §4.1, §4.2 | Model mode is opt-in and not measured on a live model (§5.4). Most non-bid drafts are correct skeletons that place fewer than half of the stated facts (0.229 overall, §5.2). |
| **Measurable business impact** | Measured today, on the machine side: the bundled tender reviewed into 14 files in 1.6 s, with all 72 requirements quoted; the long-frame packing list planned in 0.4 s; pieces in equal pieces out on all 50 packing fixtures. The pilot's business metrics (staff hours per tender review, clauses and conflicts found, containers planned against containers shipped) are defined in the Business Proposal §6. | §3.1, §3.2, §5.2; Business Proposal §6 | No staff time saved has been measured, and no baseline has been measured at the partner: the baselines are to be measured by the partner in weeks 1–2 of the pilot, and the targets agreed with it. No pilot, price or user count yet. |

**For an evaluation and safety review.** Five points the table can only summarise.

**Golden path and adversarial cases, side by side.** The golden path is the three flows through the real entries (§3.1–§3.3), the façade demo (§3.4, on `916971d`), 12 of 12 acceptance cases and the no-key one-shot demo (§5.2). The adversarial cases sit in the same gate:

- **Held-out sets written after the rules were frozen.** The router's 18 sentences pair drafting requests with look-alike questions that must not start a run: accuracy 1.000, 0 false runs. The verdict guard's 20 sentences mix 9 stated verdicts with look-alikes that must not be flagged, such as negated, asked, conditional or quoted ones: P 0.900, R 1.000 (`test/benchmarks/verdicts/heldout2.json`). On `916971d`, 28 unseen English sentences score 0.786 with 0 false runs, and every miss falls back to chat.
...
- **Inputs that must not produce a plan.** A list missing dimensions and a crafted CSV with a blank weight and "10/12" give needs-human rows and no container count (§3.2).
...
3. **Stop for a person.** Input a machine cannot trust halts that step: needs-human packing rows (no container count), cargo that is not conserved (`cargo_not_conserved`), an unreadable tender on the CLI (`tender_unreadable`, nothing written), an unreadable response (未能判断, "cannot judge", never 未响应, "no response"), number conflicts (`needs_review`, never resolved automatically), a façade clause on unmodelled handling ("Pending SME", from `916971d`), and an ambiguous request (a question, not a guess). On the gateway page, a packing run pauses at `await_user_confirm` before any container is loaded.
...
**After the baseline.** Every number in §3.4, the "Since this baseline" paragraph of §5.3 and row 13 below was run on `916971d` (`main` after pull request #63, 2026-09-26) in the same environment, with no key and `PYTHON_DOTENV_DISABLED=1`; the 145-check result is GitHub CI's. No other number in this document was re-run on `916971d`, so the baseline stays `d3ada11`. The criteria map after §1 repeats some of these figures and labels each one `916971d`.
