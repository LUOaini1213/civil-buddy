...
1. ~~`steps` 模式不读点名的文件~~ 已关：任务里点名文件夹里的装箱单就由装箱引擎真算（缺重量/尺寸的行逐条列出、不给柜数；`can_fit=False` 按失败说；点了两份表或没点名都不猜），点名的招标文件才是被解析的对象，同时点名响应文件则逐条对照，角色只从文件名读、读不出就不猜。还差的是路由：「编一份…交底」这类说法在 `steps` 下仍被判成提问（`task_router._ACTION` 不认「编」），模型模式不受影响。
