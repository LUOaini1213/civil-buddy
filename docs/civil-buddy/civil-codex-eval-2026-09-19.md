...
| 系统级沙箱（Seatbelt / bwrap） | `--sandbox-backend os`：工具在自我禁闭的工作进程里跑，Linux 用 Landlock + seccomp（写、起进程、网络都由内核拒绝），Windows 用 Low 完整性级别 + Job 对象（写、起进程；**网络不由内核限制**，照实写）；拿不到就拒绝执行。默认仍是应用层。macOS 没做。见 [os-sandbox.md](os-sandbox.md) | **通过**（Linux、Windows）/ 缺口（macOS；读不受限） |
| Plugins（skill + MCP 打包分发） | `civil plugin install <目录或 zip>`：SOP + 表单模板 + 知识摘录，**纯声明、不含代码**；不能顶替内置岗位，未受信任一律高风险，信任按内容哈希记在用户本人名下，模板不能下结论。见 [plugins.md](plugins.md) | **通过**（有意不同：不带 MCP 服务器和任何程序） |
| 原生 App | `civil desktop`：Tk 原生窗口（零依赖），坐在 CLI 同一套运行时上——作业文件夹、对话、逐步进度、审批对话框、打开成稿、复核、模式与沙箱开关。没有打包成安装程序，也不是网页工作台的壳。见 [desktop-app.md](desktop-app.md) | **通过**（窗口）/ 缺口（未打包、未与工作台合并） |
| 全屏 TUI、IDE 商店扩展、Cloud | 无 | **不做 / 缺口**（未变） |
...
4. 系统级沙箱已做（Linux、Windows；macOS 没做，读取不受限，Windows 不断网——见 os-sandbox.md）。Plugins 已做（声明式，见 plugins.md）。原生 App 已做（Tk 窗口，未打包，见 desktop-app.md）。商店扩展、Cloud 不做。
