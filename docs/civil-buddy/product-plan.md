...
| 岗 KB | 目录 100% / 写盘栏位 ~27% | **66/66** 四件套在盘；`test_kb_schema.py` 缺一即红 | outline 指针：construction→`scheme-11.md`，危大→`judge-card.md`。**真写盘 18/66**（+ lab-mix） |
| 工作台 66 岗 | 平台齐、栏位 18/66 | 同一套 chat/run | 其余 ~48 岗 `_draft_markdown` |
...
| `lane-lab` | 试验室 | 3 | 配比/取样/台账栏；无报告号不编 | **1/3** lab-mix |
...
| 15 | lab-mix | ✅ 四层目录；无试验数据则施工配比整节待填（T033 进行中） |
| 16+ | 其余 ~48 岗 | 骨架；下一刀在 post-horizon（已富岗勿再当缺口） |
...
**T033 · lab 进行中：下一岗 lab-sample__list。**

lab-mix ✅：四层目录；无试验数据则施工配比整节待填。不编 kg/m³。chat 仍不写盘。T033 其余岗 lab-sample → lab-record **未做**，不得一行勾完。

T033 下一岗：lab-sample 出类别｜部位｜见证人空｜升级路径；组数 [A001]。
...
| post-horizon construction「下一刀=十一章接 turn」 | S2 **已做** md；fill_scheme / `docx_pending` 已接（T005 ✅） | 岗栏下一刀 = T033 lab-sample（mix ✅） |
...
T030–T047 是**批次合同**（T030 / T031 / T032 ✅、lab-mix ✅ 后约 **48** 岗）。**一行不得一次勾完**；每岗一 commit。细节读 post-horizon 该 id。
...
T001 → T021 → T023 → T003 → T007 → T008 → T002 → T004 → T006 → T005 → T014 → T011 → T010 → T020+T024 → T030 → T031 → T032 ✅ → **T033**（lab-mix ✅，下一岗 sample）→ T034…T047（T050 / T052 / T062 文档与联测已 ✅，不占岗栏）。
