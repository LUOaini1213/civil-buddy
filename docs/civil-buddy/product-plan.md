...
| 岗 KB | 目录 100% / 写盘栏位 ~47% | **66/66** 四件套在盘；`test_kb_schema.py` 缺一即红 | outline 指针：construction→`scheme-11.md`，危大→`judge-card.md`。**真写盘 31/66**（+ proc-vendor） |
| 工作台 66 岗 | 平台齐、栏位 31/66 | 同一套 chat/run | 其余 ~35 岗 `_draft_markdown` |
...
| 29+ | 其余 ~35 岗 | 骨架；下一刀在 post-horizon（已富岗勿再当缺口） |
...
**T038 · finance 进行中：下一岗 finance-book__check。**

T037 ✅：proc-plan / proc-compare / proc-vendor 均已做。chat 仍不写盘。T038 起不得一行勾完。

T038 下一岗：finance-book 出报销勾选+科目对照+对账缺口，金额 [A001]。
...
| post-horizon construction「下一刀=十一章接 turn」 | S2 **已做** md；fill_scheme / `docx_pending` 已接（T005 ✅） | 岗栏下一刀 = T038 finance-book（T037 ✅） |
...
T030–T047 是**批次合同**（T030–T037 ✅ 后约 **35** 岗）。**一行不得一次勾完**；每岗一 commit。细节读 post-horizon 该 id。
...
T001 → T021 → T023 → T003 → T007 → T008 → T002 → T004 → T006 → T005 → T014 → T011 → T010 → T020+T024 → T030 → T031 → T032 ✅ → T033 ✅ → T034 ✅ → T035 ✅ → T036 ✅ → T037 ✅ → **T038**（下一岗 finance-book）→ T039…T047（T050 / T052 / T062 文档与联测已 ✅，不占岗栏）。
