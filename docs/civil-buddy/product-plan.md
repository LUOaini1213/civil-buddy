...
**怎么用：** 改产品前先读 §1 边界、§10 不做、§13 联网口径。开工只取 **§15 主链头指针**（此刻 = §11 的 T035）。岗栏位细节可读 post-horizon 该 id；已做/未做以 §7 / §15 为准。  
...
| 岗 KB | 目录 100% / 写盘栏位 ~36% | **66/66** 四件套在盘；`test_kb_schema.py` 缺一即红 | outline 指针：construction→`scheme-11.md`，危大→`judge-card.md`。**真写盘 24/66**（+ quality / env） |
| 工作台 66 岗 | 平台齐、栏位 24/66 | 同一套 chat/run | 其余 ~42 岗 `_draft_markdown` |
...
| `lane-hse` | 安质环 | 4 | 交底/质量/环保/应急草稿；SG 走 WSH 标题 | **3/4** brief + quality + env |
...
| 21 | env | ✅ 扬尘/弃土/污水/夜间/市容五行，限值 UNSPECIFIED（T035 进行中） |
| 22+ | 其余 ~42 岗 | 骨架；下一刀在 post-horizon（已富岗勿再当缺口） |
...
K4「进行中」不是可抢的下一号。P 车道（T035+）不得在主链头指针未走到 T035 时开工。T001 / T030 / T031 / T032 / T033 / T034 已 ✅。
...
**T035 · hse 进行中：下一岗 emergency__plan。**

quality ✅、env ✅。chat 仍不写盘。T035 其余岗 emergency **未做**，不得一行勾完。

T035 下一岗：emergency 出综合目录+用户点名专项+演练表头，电话医院待填。
...
| K1「目录齐但无闸」 | 当时 64/66；只缺 construction 与 method-hazard 的 `outline.md` | 已做 T001 ✅；§11 现为 T035 |
| post-horizon bid 三岗「下一刀=handoff/gaps/评分点」 | P1-1 **已做** | 以本文 §7 为准；horizon 文当历史下一刀 |
| post-horizon construction「下一刀=十一章接 turn」 | S2 **已做** md；fill_scheme / `docx_pending` 已接（T005 ✅） | 岗栏下一刀 = T035 emergency（quality/env ✅） |
...
T030–T047 是**批次合同**（T030–T034 ✅、T035 brief/quality/env ✅ 后约 **42** 岗）。**一行不得一次勾完**；每岗一 commit。细节读 post-horizon 该 id。
...
T001 → T021 → T023 → T003 → T007 → T008 → T002 → T004 → T006 → T005 → T014 → T011 → T010 → T020+T024 → T030 → T031 → T032 ✅ → T033 ✅ → T034 ✅ → **T035**（brief/quality/env ✅，下一岗 emergency）→ T036…T047（T050 / T052 / T062 文档与联测已 ✅，不占岗栏）。
