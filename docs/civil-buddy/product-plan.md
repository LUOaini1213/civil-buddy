...
| 岗 KB | 目录 100% / 写盘栏位 ~39% | **66/66** 四件套在盘；`test_kb_schema.py` 缺一即红 | outline 指针：construction→`scheme-11.md`，危大→`judge-card.md`。**真写盘 26/66**（+ equip） |
| 工作台 66 岗 | 平台齐、栏位 26/66 | 同一套 chat/run | 其余 ~40 岗 `_draft_markdown` |
...
| `lane-plant` | 物机 | 4 | pack-ship 投影 solver；设备/仓/现场料栏 | **2/4** pack-ship + equip |
...
| 23 | equip | ✅ 只抄设备名与已给证件；无证件不编进场结论（T036 进行中） |
| 24+ | 其余 ~40 岗 | 骨架；下一刀在 post-horizon（已富岗勿再当缺口） |
...
**T036 · plant 进行中：下一岗 warehouse__log。**

equip ✅：只抄用户设备名与已给证件；无证件不编进场结论。chat 仍不写盘。T036 其余岗 warehouse → material-site **未做**（pack-ship 已富跳过），不得一行勾完。

T036 下一岗：warehouse 按行抄收发原文；有数只抄、无数 TBD；无盘点不编盈亏。
...
| post-horizon construction「下一刀=十一章接 turn」 | S2 **已做** md；fill_scheme / `docx_pending` 已接（T005 ✅） | 岗栏下一刀 = T036 warehouse（equip ✅） |
...
T030–T047 是**批次合同**（T030–T035 ✅、equip ✅ 后约 **40** 岗）。**一行不得一次勾完**；每岗一 commit。细节读 post-horizon 该 id。
...
T001 → T021 → T023 → T003 → T007 → T008 → T002 → T004 → T006 → T005 → T014 → T011 → T010 → T020+T024 → T030 → T031 → T032 ✅ → T033 ✅ → T034 ✅ → T035 ✅ → **T036**（equip ✅，下一岗 warehouse；pack-ship 已富跳过）→ T037…T047（T050 / T052 / T062 文档与联测已 ✅，不占岗栏）。
