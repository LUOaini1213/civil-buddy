...
| 岗 KB | 目录 100% / 写盘栏位 ~24% | **66/66** 四件套在盘；`test_kb_schema.py` 缺一即红 | outline 指针：construction→`scheme-11.md`，危大→`judge-card.md`。**真写盘 16/66**（+ plan-lookahead） |
| 工作台 66 岗 | 平台齐、栏位 16/66 | 同一套 chat/run | 其余 ~50 岗 `_draft_markdown` |
...
| `lane-planning` | 计划 | 3 | 总控/近看/资源栏位；无进度数据不编工期 | **2/3** plan-master + plan-lookahead |
...
| 13 | plan-lookahead | ✅ 四周表；制约未清不得写入本周承诺（T032 进行中） |
| 14+ | 其余 ~50 岗 | 骨架；下一刀在 post-horizon（已富岗勿再当缺口） |
...
**T032 · planning 进行中：下一岗 plan-resource__peak。**

plan-lookahead ✅：四周表；制约未清不得写入本周承诺。不编风速限值、不写可以复工。chat 仍不写盘。T032 其余岗 plan-resource **未做**，不得一行勾完。

T032 下一岗：plan-resource 拆劳动力｜机具｜材料三表，数量待填。
...
| post-horizon construction「下一刀=十一章接 turn」 | S2 **已做** md；fill_scheme / `docx_pending` 已接（T005 ✅） | 岗栏下一刀 = T032 plan-resource（lookahead ✅） |
...
T030–T047 是**批次合同**（T030 / T031 ✅、plan-master ✅、plan-lookahead ✅ 后约 **50** 岗）。**一行不得一次勾完**；每岗一 commit。细节读 post-horizon 该 id。
...
T001 → T021 → T023 → T003 → T007 → T008 → T002 → T004 → T006 → T005 → T014 → T011 → T010 → T020+T024 → T030 → T031 → **T032**（plan-master ✅，lookahead ✅，下一岗 resource）→ T033…T047（T050 / T052 / T062 文档与联测已 ✅，不占岗栏）。
