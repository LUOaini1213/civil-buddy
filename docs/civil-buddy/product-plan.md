...
| 岗 KB | 目录 100% / 写盘栏位 ~38% | **66/66** 四件套在盘；`test_kb_schema.py` 缺一即红 | outline 指针：construction→`scheme-11.md`，危大→`judge-card.md`。**真写盘 25/66**（+ emergency） |
| 工作台 66 岗 | 平台齐、栏位 25/66 | 同一套 chat/run | 其余 ~41 岗 `_draft_markdown` |
...
| 23+ | 其余 ~41 岗 | 骨架；下一刀在 post-horizon（已富岗勿再当缺口） |
...
**T036 · plant 进行中：下一岗 equip__ledger。**

T035 ✅：safety-brief / quality / env / emergency 均已做。chat 仍不写盘。T036 起不得一行勾完。pack-ship 已富，本批跳过。

T036 下一岗：equip 写出与工作台同表头台账，只抄用户设备名与已给证件。
...
| post-horizon construction「下一刀=十一章接 turn」 | S2 **已做** md；fill_scheme / `docx_pending` 已接（T005 ✅） | 岗栏下一刀 = T036 equip（T035 ✅） |
...
T030–T047 是**批次合同**（T030–T035 ✅ 后约 **41** 岗）。**一行不得一次勾完**；每岗一 commit。细节读 post-horizon 该 id。
...
T001 → T021 → T023 → T003 → T007 → T008 → T002 → T004 → T006 → T005 → T014 → T011 → T010 → T020+T024 → T030 → T031 → T032 ✅ → T033 ✅ → T034 ✅ → T035 ✅ → **T036**（下一岗 equip；pack-ship 已富跳过）→ T037…T047（T050 / T052 / T062 文档与联测已 ✅，不占岗栏）。
