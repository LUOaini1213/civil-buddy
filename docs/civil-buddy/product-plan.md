...
| 岗 KB | 目录 100% / 写盘栏位 ~15% | **66/66** 四件套在盘；`test_kb_schema.py` 缺一即红 | outline 指针：construction→`scheme-11.md`，危大→`judge-card.md`。**真写盘 10/66**（bid×3 + pack-ship + construction + 危大 + tax + cost + survey + dispatch） |
| 工作台 66 岗 | 平台齐、栏位 10/66 | 同一套 chat/run | 其余 ~56 岗 `_draft_markdown` |
...
| 8+ | 其余 ~56 岗 | 骨架；下一刀在 post-horizon（已富岗勿再当缺口） |
...
**T031 · commercial：variation → claim → subcontract → interim（cost 已见 T006）。**

T030 ✅：survey 只抄已给点号（无则表头+[A001]）；dispatch 按 outline 十一章落表头，敏感作业只列名、判定交 method-hazard。chat 仍不写盘。T031–T047 仍是批次合同，**一行不得一次勾完**。

T031：一次只做一岗。variation 先。验收：该岗 run 产物含 outline 栏位名。
...
T030–T047 是**批次合同**（T030 ✅ 后约 **56** 岗，已扣 bid×3 / pack-ship / construction / 危大 / tax / cost / survey / dispatch）。**一行不得一次勾完**；每岗一 commit。细节读 post-horizon 该 id。
...
T001 → T021 → T023 → T003 → T007 → T008 → T002 → T004 → T006 → T005 → T014 → T011 → T010 → T020+T024 → T030 → **T031**（variation 先）→ T032…T047（T050 / T052 / T062 文档与联测已 ✅，不占岗栏）。
