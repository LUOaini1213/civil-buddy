...
**怎么用：** 改产品前先读 §1 边界、§10 不做、§13 联网口径。开工只取 **§15 主链头指针**（此刻 = §11 的 T033）。岗栏位细节可读 post-horizon 该 id；已做/未做以 §7 / §15 为准。  
...
| 岗 KB | 目录 100% / 写盘栏位 ~29% | **66/66** 四件套在盘；`test_kb_schema.py` 缺一即红 | outline 指针：construction→`scheme-11.md`，危大→`judge-card.md`。**真写盘 19/66**（+ lab-sample） |
| 工作台 66 岗 | 平台齐、栏位 19/66 | 同一套 chat/run | 其余 ~47 岗 `_draft_markdown` |
...
| `lane-lab` | 试验室 | 3 | 配比/取样/台账栏；无报告号不编 | **2/3** lab-mix + lab-sample |
...
| 16 | lab-sample | ✅ 类别｜部位｜见证人空｜升级路径；组数 [A001]（T033 进行中） |
| 17+ | 其余 ~47 岗 | 骨架；下一刀在 post-horizon（已富岗勿再当缺口） |
...
K4「进行中」不是可抢的下一号。P 车道（T033+）不得在主链头指针未走到 T033 时开工。T001 / T030 / T031 / T032 已 ✅。
...
**T033 · lab 进行中：下一岗 lab-record__ledger。**

lab-sample ✅：类别｜部位｜见证人空｜升级路径；组数 [A001]。chat 仍不写盘。T033 其余岗 lab-record **未做**，不得一行勾完。

T033 下一岗：lab-record 加报告编号待核｜仪器检定｜结论待填。
...
| K1「目录齐但无闸」 | 当时 64/66；只缺 construction 与 method-hazard 的 `outline.md` | 已做 T001 ✅；§11 现为 T033 |
| post-horizon bid 三岗「下一刀=handoff/gaps/评分点」 | P1-1 **已做** | 以本文 §7 为准；horizon 文当历史下一刀 |
| post-horizon construction「下一刀=十一章接 turn」 | S2 **已做** md；fill_scheme / `docx_pending` 已接（T005 ✅） | 岗栏下一刀 = T033 lab-record（sample ✅） |
...
T030–T047 是**批次合同**（T030 / T031 / T032 ✅、lab-mix ✅、lab-sample ✅ 后约 **47** 岗）。**一行不得一次勾完**；每岗一 commit。细节读 post-horizon 该 id。
...
T001 → T021 → T023 → T003 → T007 → T008 → T002 → T004 → T006 → T005 → T014 → T011 → T010 → T020+T024 → T030 → T031 → T032 ✅ → **T033**（mix ✅，sample ✅，下一岗 record）→ T034…T047（T050 / T052 / T062 文档与联测已 ✅，不占岗栏）。
