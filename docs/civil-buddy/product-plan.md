...
| 岗 KB | 目录 100% / 写盘栏位 ~26% | **66/66** 四件套在盘；`test_kb_schema.py` 缺一即红 | outline 指针：construction→`scheme-11.md`，危大→`judge-card.md`。**真写盘 17/66**（+ plan-resource） |
| 工作台 66 岗 | 平台齐、栏位 17/66 | 同一套 chat/run | 其余 ~49 岗 `_draft_markdown` |
...
| 15+ | 其余 ~49 岗 | 骨架；下一刀在 post-horizon（已富岗勿再当缺口） |
...
**T033 · lab 进行中：下一岗 lab-mix__report。**

T032 ✅：plan-master / plan-lookahead / plan-resource 均已做。chat 仍不写盘。T033 起不得一行勾完。

T033 下一岗：lab-mix 四层目录；无试验数据则施工配比整节待填。
...
| post-horizon construction「下一刀=十一章接 turn」 | S2 **已做** md；fill_scheme / `docx_pending` 已接（T005 ✅） | 岗栏下一刀 = T033 lab-mix（T032 ✅） |
...
T030–T047 是**批次合同**（T030 / T031 / T032 ✅ 后约 **49** 岗）。**一行不得一次勾完**；每岗一 commit。细节读 post-horizon 该 id。
...
T001 → T021 → T023 → T003 → T007 → T008 → T002 → T004 → T006 → T005 → T014 → T011 → T010 → T020+T024 → T030 → T031 → T032 ✅ → **T033**（下一岗 lab-mix）→ T034…T047（T050 / T052 / T062 文档与联测已 ✅，不占岗栏）。
