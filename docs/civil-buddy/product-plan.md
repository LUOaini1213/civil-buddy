...
| 岗 KB | 目录 100% / 写盘栏位 ~33% | **66/66** 四件套在盘；`test_kb_schema.py` 缺一即红 | outline 指针：construction→`scheme-11.md`，危大→`judge-card.md`。**真写盘 22/66**（+ safety-brief） |
| 工作台 66 岗 | 平台齐、栏位 22/66 | 同一套 chat/run | 其余 ~44 岗 `_draft_markdown` |
...
| `lane-hse` | 安质环 | 4 | 交底/质量/环保/应急草稿；SG 走 WSH 标题 | **1/4** safety-brief |
...
| 19 | safety-brief | ✅ 11 栏；毫米/电话 [A001]（T035 进行中） |
| 20+ | 其余 ~44 岗 | 骨架；下一刀在 post-horizon（已富岗勿再当缺口） |
...
**T035 · hse 进行中：下一岗 quality__lot。**

safety-brief ✅：11 栏；毫米/电话 [A001]。高风险须确认句。chat 仍不写盘。T035 其余岗 quality → env → emergency **未做**，不得一行勾完。

T035 下一岗：quality 出主控｜一般｜隐蔽三表，结果=未检。
...
| post-horizon construction「下一刀=十一章接 turn」 | S2 **已做** md；fill_scheme / `docx_pending` 已接（T005 ✅） | 岗栏下一刀 = T035 quality（brief ✅） |
...
T030–T047 是**批次合同**（T030–T034 ✅、safety-brief ✅ 后约 **44** 岗）。**一行不得一次勾完**；每岗一 commit。细节读 post-horizon 该 id。
...
T001 → T021 → T023 → T003 → T007 → T008 → T002 → T004 → T006 → T005 → T014 → T011 → T010 → T020+T024 → T030 → T031 → T032 ✅ → T033 ✅ → T034 ✅ → **T035**（safety-brief ✅，下一岗 quality）→ T036…T047（T050 / T052 / T062 文档与联测已 ✅，不占岗栏）。
