...
> **产品主线 C（2026-08 起）**：项目投标应答 + 交付链路（幕墙窄行业）  
> 装柜 A+B 冻结数字仍作 **交付引擎证据**；默认 UI 是投标矩阵，不是工程装柜台。

**Harness v0.6.4** · **13 agents** · 主路径 **`agent_mode=steps`**  
**交付引擎演示**：可关「演示自动确认」，露出 `phase=await_user_confirm`  
**冻结日（A+B 数字）**：2026-08-05 · commit 基线 `7a0744d`

---

## 主线 C · 3 分钟口播（优先）

> 业主招标要求包装出运、柜型与 CTU 重心。Agent **先抽条款**做成合规矩阵（owner / risk / 应答落点），再用 **A 成箱 + B 拼柜 tools** 生成 can_fit / mid50 作交付证据；资质与废标项仍 **人签**。  
> **不**自动编造 20 万字标书。

| 步 | 操作 | 看点 |
|----|------|------|
| 1 | 打开 `http://127.0.0.1:8000/` | 标题「投标应答 · 交付链路」 |
| 2 | 样例已填 → **解析并生成交付证据** | 矩阵 covered + mid50 非空 |
| 3 | **人工待办** + **复制应答草稿** | 导出 Markdown；商务/资质 human_required |
| 4 | （可选）`/workbench` | 旧装柜工程台，作深度证据 |

```bash
python scripts/test_tender_parse.py
python scripts/test_tender_delivery_api.py
```
