...
| 能力 | 状态 |
|------|------|
| 多柜策略 / 446t 历史 25 柜 | 旧档。客户文件不在仓库，不能复验；29 柜基线已废弃 |
...
3. 非标或表上传 G1  
4. 备份：不要口播 446 t / 25 柜。客户文件 `output/cases_446t/materials.json` 不在仓库，不能复验；29 柜基线已废弃。改报净仓长框架：9 箱、装得下、23800 kg 守恒。  
