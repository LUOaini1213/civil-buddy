...
| 后台取消 | 停止按钮请求取消；正在运行的本地工具完成当前调用后不再执行后续步骤。真实模型连接被关闭；浏览器 HTTP 断开同样收尾并释放会话，部分回复和已写文件可恢复。`test_workbench_cancel.py` 9 项通过，包含真实阻塞 socket。 |
...
- 高风险确认使用 `confirm_ok` 布尔协议；界面要求键入确认句，JSON 接口拒绝字符串、数值等强制转换，纯问题和未确认的高风险请求不写文书。
